    ...
    trait: [
        ...
        // deftness: {
            ...
            //     bow: {
                //         name: "Bow",
                //         dieCount: 0,
                //         dieSides: player2.deftness.dieSides,
                //         type: "trait",
    //     },
            ...
            attribute: [
                {
                    ...
                    dieCount: 8,
                    dieSides: 4,
                    // set to integer for now. In order to have it set to its parent attribute dieSides, 
                    //must set dieSides after initialization (closing final curly bracket on parent object)
                }
            ]
        } 
    ] 
  };
  console.log(obj.profile.user.name, obj.relations[0], obj.relations[0].name, obj.relations[1].attribute[0].dieSides); 
  // ['xyz', { name: 'one' }, undefined]

console.log(player2.trait.cognition);

for (var traitCount in player2.trait) {
    console.log("here is the name of a trait: " + player2.trait[traitCount].name);
    for (attributeCount in player2.trait[traitCount].attribute) {
        console.log("here is the name of an attribute: " + player2.trait[traitCount].attribute[attributeCount].name)
    }
}
